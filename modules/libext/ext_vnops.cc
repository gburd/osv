...
ext_readlink(vnode_t *vp, uio_t *uio)
{
    ...
    uint64_t fsize = ext4_inode_get_size(&fs->sb, inode_ref._ref.inode);
    // An offset at or past the target end reads nothing (same as the slow path
    // below); guard it here too so the fast path is consistent and a non-zero
    // offset cannot copy from the start of the inline target.

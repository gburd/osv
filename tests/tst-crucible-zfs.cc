...
bool run_io_roundtrip(size_t bytes, io_result *out)
{
    ...
    if (!buf) {
        FAIL("malloc(%zu) failed", bytes);
        return false;
    }
    for (size_t i = 0; i < bytes; i++) {
        buf[i] = static_cast<uint8_t>((i * 31u + (i >> 11)) ^ 0xA7);
    }
    ...
    if (n != static_cast<ssize_t>(bytes)) {
        FAIL("write returned %zd (errno=%d %s)", n, errno, strerror(errno));
        close(fd); free(buf);
        return false;
    }
    if (fsync(fd) != 0) {
    ...
    uint8_t *rbuf = static_cast<uint8_t *>(malloc(bytes));
    n = read(fd, rbuf, bytes);
    auto r1 = std::chrono::high_resolution_clock::now();
    out->read_secs = std::chrono::duration<double>(r1 - r0).count();

    if (n != static_cast<ssize_t>(bytes)) {
        FAIL("read returned %zd", n);
        close(fd); free(buf); free(rbuf);
        return false;
    }
    if (memcmp(buf, rbuf, bytes) != 0) {
        size_t first_diff = 0;
        for (size_t i = 0; i < bytes; i++) {
            if (buf[i] != rbuf[i]) { first_diff = i; break; }
        }
        printf("  data mismatch starting at offset %zu\n", first_diff);
        printf("  expected:");
        for (size_t i = first_diff; i < first_diff + 16 && i < bytes; i++)
            printf(" %02x", buf[i]);
        printf("\n  got:     ");
        for (size_t i = first_diff; i < first_diff + 16 && i < bytes; i++)
            printf(" %02x", rbuf[i]);
